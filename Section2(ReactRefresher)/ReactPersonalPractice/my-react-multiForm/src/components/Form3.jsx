...
function Form3() {
	...
	const addAnotherInput = ()=>{
		...
		setFormInput((prev) => {
			const previousName = prev[prev.length - 1].name
				? `${prev[prev.length - 1].name.slice(0,-2)} ${prev.length+1}`
				: ''
			const previousAge = prev[prev.length - 1].age
				? prev[prev.length - 1].age
				: ''
			const previousAddress = prev[prev.length - 1].address
				? prev[prev.length - 1].address
				: ''
			return [
				...prev,
				{ name: previousName, age: previousAge, address: previousAddress },
			]})
	}
